		...
		switch ($_SERVER['REQUEST_METHOD']) {
			case 'POST':
				...
				if (isset($postData->id)) {
					...
					$datastore->execute($stmt, array('taskText'=>$postData->task, 'id'=>$id));
				} else {
					...
					$id = $datastore->lastInsertId();
				}
				return $id;
			case 'GET':
				$start = normaliseDate($params[1], 'start');
				$end = (count($params) > 2) ? normaliseDate($params[2], 'end') : normaliseDate($params[1], 'end');
				return json_encode(Tasks::getTasks($start, $end));
			case 'DELETE':
				$stmt = $datastore->prepare('DELETE FROM tasks WHERE id=:id');
				$datastore->execute($stmt, array('id'=>$params[1]));
				return;
			default:
